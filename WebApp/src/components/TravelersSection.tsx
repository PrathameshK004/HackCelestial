...
export const TravelersSection: React.FC<TravelersSectionProps> = ({
  ...
  const handleCopyInviteLink = (traveler: Traveler) => {
    if (traveler.isRegistered === false) {
      window.alert('This traveler must create and verify a Triptual account before an email invitation can be sent.');
      return;
    }
    const LIVE_APP_DOMAIN = 'http://localhost:5173';
