...
export function getSupportSocket(): Socket {
  if (!socket) {
    ...
    socket.on("connect", () => {
      joinedTicketNumbers.forEach((ticketNumber) =>
...
export function joinSupportTicket(ticketNumber: string): () => void {
  ...
  if (activeSocket.connected)
    activeSocket.emit("join:ticket", cleanTicketNumber);
  return () => {
...
export function onSupportTicketMessage(
...
export function onSupportTicketStatus(
  callback: (payload: any) => void,
): () => void {
  const activeSocket = getSupportSocket();
...
export function onSupportSocketReconnect(callback: () => void): () => void {
  const activeSocket = getSupportSocket();
  activeSocket.on("connect", callback);
  if (activeSocket.connected) callback();
  return () => activeSocket.off("connect", callback);
}
